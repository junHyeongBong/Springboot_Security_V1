package com.pg.security1.config;
...
import com.pg.security1.config.oauth.PrincipalOauth2UserService;
...
public class SecurityConfig extends WebSecurityConfigurerAdapter{
	...
	protected void configure(HttpSecurity http) throws Exception {
		...
		http.authorizeRequests()
			...
			.formLogin()
			.loginPage("/loginForm")
			.loginProcessingUrl("/login") // /login 주소가 호출이 되면 security가 낚아채서 대신 로그인진행 controller에 /login을 따로 안만들어주어도된다.
			.defaultSuccessUrl("/")
			.and()
			.oauth2Login()
